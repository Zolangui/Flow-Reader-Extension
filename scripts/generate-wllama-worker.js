...
function resolveWllamaRoot() {
    ...
    const pkgJson = require.resolve('@wllama/wllama/package.json', { paths: [readerDir, root] });
    return path.dirname(pkgJson);
}

const wllamaRoot = resolveWllamaRoot();
const generatedPath = path.join(wllamaRoot, 'src', 'workers-code', 'generated.ts');
const outPath = path.join(root, 'apps/reader/public/wasm/wllama.worker.js');
const opfsOutPath = path.join(root, 'apps/reader/public/wasm/wllama.opfs.worker.js');
const wasmSingleSrc = path.join(wllamaRoot, 'esm', 'single-thread', 'wllama.wasm');
const wasmMultiSrc = path.join(wllamaRoot, 'esm', 'multi-thread', 'wllama.wasm');
const wasmSingleOut = path.join(root, 'apps/reader/public/wasm/wllama-single.wasm');
const wasmMultiOut = path.join(root, 'apps/reader/public/wasm/wllama-multi.wasm');
...
const wasmSingleAliasOut = path.join(root, 'apps/reader/public/wasm/wllama.wasm');
...
if (!LLAMA_CPP_WORKER_CODE || !WLLAMA_SINGLE_THREAD_CODE || !OPFS_UTILS_WORKER_CODE) {
...
const instantiateAsyncStart = 'async function instantiateAsync(binary,binaryFile,imports){'
...
if (startIdx !== -1) {
    ...
    if (endIdx !== -1) {
        ...
        mainModuleCode = mainModuleCode.slice(0, startIdx) + replacement + mainModuleCode.slice(afterIdx)
    } else {
        console.warn('WARN: Could not locate end of instantiateAsync() in wllama single-thread code; leaving fetch path as-is')
    }
} else {
    console.warn('WARN: Could not locate instantiateAsync() in wllama single-thread code; leaving fetch path as-is')
}
...
if (!fs.existsSync(wasmSingleSrc) || !fs.existsSync(wasmMultiSrc)) {
    console.error('Missing wllama wasm files at', wasmSingleSrc, 'or', wasmMultiSrc);
    process.exit(1);
}
