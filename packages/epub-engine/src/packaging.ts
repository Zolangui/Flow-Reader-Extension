...
class Packaging {
  ...
  parseSpine(
    spineXml: Element,
    _manifest: PackagingManifestObject,
  ): PackagingSpineItem[] {
  ...
  findUniqueIdentifier(packageXml: Document): string {
    ...
    const localName =
      colon === -1
        ...
        : identifier.localName.slice(colon + 1)
    const isDc =
      identifier.namespaceURI === DC_NS ||
      (colon !== -1 && identifier.localName.slice(0, colon) === 'dc')
  ...
  load(json: WebPubManifest): PackagingObject & { toc: NavItem[] } {
    ...
    json.resources.forEach(
      (item: PackagingManifestItem & { rel?: string[] }, index: number) => {
