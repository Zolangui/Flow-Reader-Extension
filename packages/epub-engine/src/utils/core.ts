 ...
 * @memberof Core
 */
export const microTick: (cb: FrameRequestCallback) => number = (cb) => {
...
export function defaults<T extends object>(obj: T, ...sources: object[]): T {
  for (let i = 0; i < sources.length; i++) {
    ...
    for (const prop in source) {
      if ((obj as Record<string, unknown>)[prop] === void 0)
...
export function locationOf<T>(
  ...
  if (!compareFunction) {
    compareFunction = function (a: T, b: T): number {
      if ((a as unknown as string | number) > (b as unknown as string | number))
        return 1
      if ((a as unknown as string | number) < (b as unknown as string | number))
        return -1
      return 0
...
export function isKnownRequestType(type?: string): boolean {
  return (
    !!type && (['json', 'xhtml', 'html', 'htm'].includes(type) || isXml(type))
  )
}
...
export function parse(markup: string, mime: string): Document {
  ...
  const ParserCtor =
    configuredDOMParser ??
    (globalThis.DOMParser as DOMParserConstructor | undefined)
  if (!ParserCtor) {
    throw new EpubError(
      'DOMParser is unavailable in this environment; import from "@likecoin/epub-ts/node" or call setDOMParser() to provide one.',
    )
  }
  return new ParserCtor().parseFromString(
    markup,
...
export function treeWalker(
  ...
  // EPUB source documents and iframe documents can belong to a different
  // realm from the reader shell. Firefox rejects a foreign root passed to the
  // shell's `document.createTreeWalker()`, so always use the root's owner.
  const ownerDocument =
    root.nodeType === 9 ? (root as Document) : root.ownerDocument
  if (!ownerDocument) {
    throw new TypeError('TreeWalker root does not belong to a Document')
  }
  const treeWalker = ownerDocument.createTreeWalker(root, filter, null)
  let node
...
const _withResolvers: (<U>() => WithResolversResult<U>) | undefined =
  ...
  'function'
    ? (
        Promise as unknown as { withResolvers: <U>() => WithResolversResult<U> }
...
export function querySelectorByType(
  ...
  for (let i = 0; i < elements.length; i++) {
    if (
      elements[i]!.getAttributeNS('http://www.idpf.org/2007/ops', 'type') ===
        type ||
...
export class RangeObject {
  ...
  selectNode(referenceNode: Node): void {
    ...
    const index = Array.from(parent.childNodes as ArrayLike<Node>).indexOf(
      referenceNode,
    )
