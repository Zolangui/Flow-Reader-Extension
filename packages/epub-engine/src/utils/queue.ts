    ...
    if (this._q.length && !this.paused) {
      ...
      if (task) {
        ...
        try {
          ...
          // A synchronous exception must behave like a rejected async task:
          // reject the promise returned by enqueue(), but settle dequeue()
          // so run()/flush() can continue with the next queued item.
        ...
        // Task is a promise
        // The producer keeps the original promise returned by enqueue().
        // This settlement-only branch prevents a rejected gate promise
        // (for example Book.opened) from permanently stalling the queue.
        return inwait.promise.then(
          () => undefined,
          () => undefined,
        )
      }
