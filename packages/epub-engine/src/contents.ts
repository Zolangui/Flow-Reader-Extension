...
import { markPresentationRuntimeNode } from './presentation-marker'
...
class Contents implements IEventEmitter<ContentsEvents> {
  ...
  _layoutStyle!: string
  private readonly _pendingStylesheets = new Map<
    string,
   ...
   */
  viewport(
    options?: Partial<Record<keyof ViewportSettings, string | number>>,
  ): ViewportSettings {
    ...
    settings = defaults(
      options ??
        ({} as Partial<Record<keyof ViewportSettings, string | number>>),
      parsed as Partial<Record<keyof ViewportSettings, string | number>>,
    )
          ...
          if (isWebkit) {
            ...
            try {
              if (container.nodeType === ELEMENT_NODE) {
                ...
                range.startOffset + 2 <
                (container as CharacterData).length
              ) {
  ...
  addStylesheet(src: string): Promise<boolean> {
    if (!this.document || !this.active) return Promise.resolve(false)

    let key = src
    try {
      key = new URL(src, this.document.baseURI).href
    } catch {
      // Keep malformed/custom scheme URLs comparable without interpolating
      // them into a CSS selector, which could throw a SyntaxError.
    }

    const pending = this._pendingStylesheets.get(key)
    ...
    ).find((link) => link.href === key || link.getAttribute('href') === src)
    if (existing) return Promise.resolve(true)

    const stylesheet = this.document.createElement('link')
    markPresentationRuntimeNode(stylesheet)
    stylesheet.type = 'text/css'
    stylesheet.rel = 'stylesheet'
    stylesheet.href = src
    stylesheet.setAttribute('data-lumen-reader-style', 'true')

    let settled = false
    let timeout: ReturnType<typeof setTimeout> | undefined
    let resolvePromise: (value: boolean) => void = () => undefined
    const finish = (loaded: boolean, remove = false): void => {
      if (settled) return
      settled = true
      if (timeout !== undefined) clearTimeout(timeout)
      stylesheet.onload = null
      stylesheet.onerror = null
      if (remove) stylesheet.remove()
      if (this._pendingStylesheets.get(key)?.promise === promise) {
        this._pendingStylesheets.delete(key)
      }
    ...
    this._pendingStylesheets.set(key, {
      promise,
      cancel: () => finish(false, true),
    })
    stylesheet.onload = (): void => {
      // Give the browser one task to finish applying the parsed sheet.
      setTimeout(() => finish(true), 0)
    }
    stylesheet.onerror = (): void => finish(false, true)
    timeout = setTimeout(() => finish(false, true), STYLESHEET_LOAD_TIMEOUT_MS)
  ...
  _getStylesheetNode(key?: string): HTMLStyleElement | false {
    ...
    if (!styleEl) {
      ...
      markPresentationRuntimeNode(styleEl)
      styleEl.id = key
      styleEl.setAttribute('data-lumen-reader-style', 'true')
  ...
  addStylesheetRules(
    ...
    // Grab style sheet
    const styleElement = this._getStylesheetNode(key) as HTMLStyleElement
    // The documented keyed behavior is replacement, not accumulation. This
    // also makes pre-pagination preparation safely idempotent.
    ...
    if (Array.isArray(rules)) {
      ...
      for (let i = 0, rl = rules.length; i < rl; i++) {
        let j = 1,
          rule: Array<string | string[]> = rules[i]!,
          propStr = ''
  ...
  triggerSelectedEvent(selection: Selection): void {
    ...
    if (selection && selection.rangeCount > 0) {
      // Selection is a live browser object. Firefox may clear it between the
      // rangeCount read and getRangeAt() when an iframe is repaginated or
      // removed, so treat that race as an ordinary deselection.
      try {
  ...
  size(width?: number, height?: number): void {
    const viewport: Partial<Record<keyof ViewportSettings, string | number>> = {
      scale: 1.0,
      scalable: 'no',
    }
    ...
    batch.push(['margin', '0'], ['box-sizing', 'border-box'])
  ...
  scaler(scale: number, offsetX?: number, offsetY?: number): void {
    ...
    if (
      (offsetX !== undefined && offsetX >= 0) ||
      (offsetY !== undefined && offsetY >= 0)
    ) {
      translateStr =
  ...
  destroy(): void {
    this.active = false
    for (const pending of this._pendingStylesheets.values()) pending.cancel()
    this._pendingStylesheets.clear()
