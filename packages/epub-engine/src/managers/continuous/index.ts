...
class ContinuousViewManager extends DefaultViewManager {
  ...
  constructor(options: ManagerOptions) {
    ...
    this.viewSettings = {
      ...
      width: 0,
      height: 0,
      // Continuous rendering historically keeps the natural number of
      // columns; preserve that default while accepting an explicit override.
      forceEvenPages: this.settings.forceEvenPages ?? false,
  ...
  async fill(): Promise<void> {
    ...
    try {
      ...
      // A failed prefetch must not leave trimming permanently disabled.
      this._filling = false
    }
  }
  ...
  update(_offset?: number): Promise<void> {
    ...
    for (let i = 0; i < viewsLength; i++) {
      ...
      if (visibleIndices.has(i)) {
        if (!view.displayed) {
          promises.push(
            view
              ...
              .then((v: IframeView) => v.show())
              .catch((error: unknown) => {
                if (this.views.indexOf(view) !== -1) {
                  // A native shell remains useful in the scrolling buffer,
                  // but none of the failed iframe/document state does.
                  if (typeof view.unload === 'function') view.unload()
                  else this.views.remove(view)
                }
                throw error
              }),
          )
        ...
        // Keep the lightweight view shell in the scrolling buffer, but release
        // its iframe and document. `destroy()` is terminal and is reserved for
        // views removed by trim()/clear().
        this.q.enqueue(() => {
          // Third-party/custom views can still expose only the legacy
          // destroy() API. Native IframeView uses unload() so it remains
          // reusable when it returns to the scrolling buffer.
          if (typeof view.unload === 'function') {
            return view.unload()
          }
          return view.destroy?.()
        })
  ...
  check(_offsetLeft?: number, _offsetTop?: number): Promise<boolean | void> {
    ...
    const writingMode =
      this.writingMode && this.writingMode.startsWith('vertical')
        ? 'vertical'
        : 'horizontal'
    ...
    const promises = newViews.map((viewPromise) => {
      return viewPromise.then((view) => this.displayView(view))
    })
  ...
  addEventListeners(_stage?: Stage): void {
    ...
    if (this.isPaginated && this.settings.snap) {
      this.snapper = new Snap(
        this as unknown as DefaultViewManager,
        typeof this.settings.snap === 'object'
          ? (this.settings.snap as Record<string, unknown>)
          : undefined,
      )
    }
  }
