...
class Mapping {
	...
	constructor(layout: LayoutProps, direction?: string, axis?: string, dev = false, measurer?: TextMeasurer) {
		this.layout = layout;
	...
	walk(root: Node, func: (node: Node) => Node | undefined): Node | undefined {
		...
		// `root` often belongs to an EPUB iframe. Its own document must create
		// the walker; the host-page global document can reject cross-realm nodes.
		const rootDocument = root.ownerDocument || document;
		const nodeFilter = rootDocument.defaultView?.NodeFilter ?? NodeFilter;
		const treeWalker = rootDocument.createTreeWalker(root, nodeFilter.SHOW_TEXT, filter);
		let node;
		let result;
	 ...
	 * @return {Range}
	 */
	findStart(root: Node, start: number, end: number, scrollDimension?: number, noFast = false): Range {
	 ...
	 * @return {Range}
	 */
	findEnd(root: Node, start: number, end: number, scrollDimension?: number, noFast = false): Range {
		// Canvas fast path: binary search on cumulative document widths
		if (!noFast && root.nodeType === Node.ELEMENT_NODE) {
