 ...
 */
export type DOMParserConstructor = {
  new (): { parseFromString(markup: string, mimeType: string): Document }
}
...
export interface RenditionOptions {
  ...
  orientation?: string
  gap?: number
  /** Internal renderer boundary; inert until lifecycle handlers register. */
  paginationLifecycle?: PaginationLifecycle
  paginationPurpose?: PaginationViewPurpose
  /**
   * Pad an odd reflowable section with a synthetic physical leaf in a
   * two-up default manager. Defaults to true for the default manager and
   * false for the continuous manager. Layout measurement disables it and
   * lets the atlas planner own synthetic blanks instead.
   */
...
export interface ViewSettings {
  ...
  forceEvenPages?: boolean
  /**
   * Place a standalone fixed-layout leaf on a physical side, or center it.
   * This represents a synthetic leading/centering gap; ordinary paired leaves
   * do not need an override even when their spine item declares page-spread.
   */
  forcePageSpread?: 'left' | 'right' | 'center'
  /** @deprecated Use forcePageSpread. Kept for third-party View callers. */
  forceRight?: boolean
  allowScriptedContent?: boolean
  allowPopups?: boolean
  globalLayoutProperties?: GlobalLayout
