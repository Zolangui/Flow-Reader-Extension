...
describe('InlineView', () => {
  ...
  describe('display()', () => {
    ...
    it('should resolve immediately if already displayed', async () => {
      const view = createView()
      await view.display(vi.fn())
      const section = view.section as unknown as {
  ...
  describe('render()', () => {
    it('should call section.render with the request function', async () => {
      ...
      expect(
        (section as unknown as { render: ReturnType<typeof vi.fn> }).render,
    ...
    it('should call layout.format on contents', async () => {
      ...
      await view.render(vi.fn())
      expect(
        (view.layout as unknown as { format: ReturnType<typeof vi.fn> }).format,
      ).toHaveBeenCalledWith(view.contents, view.section)
    })
    ...
    it('should emit loaderror on render failure', async () => {
      const section = createMockSection()
      ;(
        section as unknown as { render: ReturnType<typeof vi.fn> }
      ).render.mockRejectedValue(new Error('fail'))
      const view = createView(section)
      const handler = vi.fn()
