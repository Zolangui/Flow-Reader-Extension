...
describe('Views', () => {
  ...
  describe('clear()', () => {
    ...
    it('also destroys a view that is still loading', () => {
      const views = createViews()
      const loadingView = createMockView(0, false)
      views.append(loadingView)

      views.clear()

      expect(loadingView.destroy).toHaveBeenCalledTimes(1)
      expect(views.container.children).toHaveLength(0)
    })

    it('removes a reparented nested view without interrupting cleanup', () => {
      const views = createViews()
      const nested = document.createElement('div')
      views.container.appendChild(nested)
      const view = createMockView(0)
      views.append(view)
      nested.appendChild(view.element)
