...
describe('Mapping', () => {
  ...
  describe('rangePairToCfiPair()', () => {
    it('should collapse start range to true and end range to false', () => {
      ...
      try {
        mapping.rangePairToCfiPair('epubcfi(/6/2!)', {
          start: startRange,
          end: endRange,
        })
      } catch {
  ...
  describe('rangeListToCfiList()', () => {
    it('should process each range pair in the list', () => {
      const mapping = new Mapping(createMockLayout())
      const spy = vi
        .spyOn(mapping, 'rangePairToCfiPair')
        .mockReturnValue({ start: 'cfi1', end: 'cfi2' })
  ...
  describe('section()', () => {
    it('should call findRanges and rangeListToCfiList', () => {
      ...
      const cfiSpy = vi
        .spyOn(mapping, 'rangeListToCfiList')
        .mockReturnValue([{ start: 'a', end: 'b' }])
  ...
  describe('page() with body content', () => {
    it('should call findStart and findEnd and return a CFI pair', () => {
      ...
      vi.spyOn(mapping, 'rangePairToCfiPair').mockReturnValue({
        start: 'epubcfi(/6/2!/4/1:0)',
        end: 'epubcfi(/6/2!/4/1:22)',
      })
  ...
  describe('page() canvas-fast-path inversion guard', () => {
    ...
    const RECOMPUTED = {
      start: 'epubcfi(walk-start)',
      end: 'epubcfi(walk-end)',
    }
    ...
    it('recomputes with the DOM walk when the fast path inverts start/end', () => {
      ...
      const findStart = vi
        .spyOn(mapping, 'findStart' as any)
        .mockImplementation((...a: unknown[]) =>
          a[4] === true ? rangeOn(p1) : rangeOn(p2),
        )
      const findEnd = vi
        .spyOn(mapping, 'findEnd' as any)
        .mockImplementation((...a: unknown[]) =>
          a[4] === true ? rangeOn(p2) : rangeOn(p1),
        )
    ...
    it('does not recompute when the fast path result is already ordered', () => {
      ...
      const findStart = vi
        .spyOn(mapping, 'findStart' as any)
        .mockReturnValue(rangeOn(p1))
      const findEnd = vi
        .spyOn(mapping, 'findEnd' as any)
        .mockReturnValue(rangeOn(p2))
      const toCfi = vi
        .spyOn(mapping, 'rangePairToCfiPair')
        .mockReturnValue(FAST_PAIR)
  ...
  describe('_canvasFindNode fast path', () => {
    it('should return null when no measurer is set', () => {
      ...
      const result = (mapping as any)._canvasFindNode(
        container,
        0,
        400,
        'start',
    ...
    function setupFastPath(
      ...
      const rectSpy = vi
        ...
        .mockImplementation(function (this: Range): DOMRect {
    ...
    it('should fall back to DOM walk when match lands at window left edge', () => {
      ...
      const { mapping, container, restore } = setupFastPath(
        'ltr',
        [
