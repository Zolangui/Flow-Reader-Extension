...
function createMockManagerOptions(
  overrides?: Partial<ManagerOptions['settings']>,
): ManagerOptions {
  return {
    view: class MockView {
...
describe('ContinuousViewManager', () => {
  describe('constructor', () => {
    ...
    it('should handle gap=0', () => {
      const manager = new ContinuousViewManager(
        createMockManagerOptions({ gap: 0 }),
      )
    ...
    it('should honor an explicit forceEvenPages override', () => {
      const manager = new ContinuousViewManager(
        createMockManagerOptions({ forceEvenPages: true }),
      )
    ...
    it('should pass method through to viewSettings', () => {
      const manager = new ContinuousViewManager(
        createMockManagerOptions({ method: 'blobUrl' }),
      )
  ...
  describe('display failure handling', () => {
    it('removes and destroys a partial view when add fails', async () => {
      ...
      await expect(manager.add({ index: 0 } as Section)).rejects.toBe(failure)
    ...
    it('resets the filling guard when a prefetch fails', async () => {
      const manager = new ContinuousViewManager(createMockManagerOptions())
      const failure = new Error('prefetch failed')
      vi.spyOn(manager, 'check').mockRejectedValue(failure)

      await expect(manager.fill()).rejects.toBe(failure)

      expect((manager as unknown as { _filling: boolean })._filling).toBe(false)
    })
  })
  ...
  describe('update()', () => {
    ...
    it('should destroy each off-screen view individually (regression: closure capture)', () => {
      ...
      manager.q.tick = (cb: FrameRequestCallback): number => {
        cb(0)
        return 0
      }
      manager.update()
      ...
      expect(
        views[0]!.destroy as ReturnType<typeof vi.fn>,
      ).toHaveBeenCalledTimes(1)
      expect(
        views[4]!.destroy as ReturnType<typeof vi.fn>,
      ).toHaveBeenCalledTimes(1)
      expect(
        views[1]!.destroy as ReturnType<typeof vi.fn>,
      ).not.toHaveBeenCalled()
      expect(
        views[2]!.destroy as ReturnType<typeof vi.fn>,
      ).not.toHaveBeenCalled()
      expect(
        views[3]!.destroy as ReturnType<typeof vi.fn>,
      ).not.toHaveBeenCalled()
    })
    ...
    it('unloads a failed buffered redisplay and propagates its error', async () => {
      const manager = new ContinuousViewManager(createMockManagerOptions())
      manager.render(document.createElement('div'), { width: 800, height: 600 })
      const failure = new Error('buffered redisplay failed')
      const view = makeMockView(0)
      view.displayed = false
      view.display = vi.fn().mockRejectedValue(failure)
      view.unload = vi.fn()
      manager.views.append(view as any)
      vi.spyOn(manager, 'isVisible').mockReturnValue(true)

      await expect(manager.update()).rejects.toBe(failure)

      expect(view.unload).toHaveBeenCalledTimes(1)
      expect(manager.views.indexOf(view as any)).toBe(0)
      manager.destroy()
    })
  })

  describe('trim()', () => {
    function makeDisplayedView(
      index: number,
      hasPrev: boolean,
      hasNext: boolean,
  ...
  describe('erase()', () => {
    ...
    function setContainerScroll(
      ...
      Object.defineProperty(manager.container, 'scrollTop', {
        value: top,
        configurable: true,
        writable: true,
      })
      Object.defineProperty(manager.container, 'scrollLeft', {
        value: left,
        configurable: true,
        writable: true,
      })
    ...
    function setupManager(overrides: {
      ...
      if (overrides.direction !== undefined)
        manager.settings.direction = overrides.direction
      if (overrides.fullsize !== undefined)
        manager.settings.fullsize = overrides.fullsize
