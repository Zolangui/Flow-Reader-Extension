...
function createMockContents(fits = true) {
  return {
...
describe('Layout', () => {
  ...
  describe('sectionLayoutName()', () => {
    it('should fall back to the global layout without a section', () => {
      ...
      expect(sectionLayoutName(undefined, 'pre-paginated')).toBe(
        'pre-paginated',
      )
    })
    ...
    it('should fall back to the global layout without rendition properties', () => {
      expect(
        sectionLayoutName(sectionWith(['page-spread-right']), 'reflowable'),
      ).toBe('reflowable')
    })
    ...
    it('should honor a per-itemref reflowable override', () => {
      expect(
        sectionLayoutName(
          sectionWith(['rendition:layout-reflowable']),
          'pre-paginated',
        ),
      ).toBe('reflowable')
    })
  ...
  describe('format()', () => {
    ...
    it('should fit a pre-paginated section of a reflowable book', () => {
      ...
      expect(contents.fit).toHaveBeenCalledWith(
        layout.columnWidth,
        layout.height,
        section,
      )
    ...
    it('should paginate a reflowable section of a pre-paginated book', () => {
      ...
      const contents = createMockContents()
      layout.format(
        contents as unknown as Contents,
        sectionWith(['rendition:layout-reflowable']),
      )
    ...
    it('should paginate when fit() declines for want of a viewport', () => {
      ...
      layout.format(
        contents as unknown as Contents,
        sectionWith(['rendition:layout-pre-paginated']),
      )
