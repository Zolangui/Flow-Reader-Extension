...
describe('Book', () => {
  describe('Unarchived', () => {
    ...
    it('should have a local coverUrl', async () => {
      expect(await book.coverUrl()).toBe(
        getFixtureUrl('/alice/OPS/images/cover_th.jpg'),
      )
    })
  })
  ...
  describe('Sub-object parity (archived epub)', () => {
    ...
    it('should have correct packaging metadata title', () => {
      expect(book.packaging.metadata.title).toBe(
  ...
  describe('Open failure', () => {
    ...
    it('recognizes an ArrayBuffer created in another realm as book data', () => {
      const input = runInNewContext('new ArrayBuffer(8)') as ArrayBuffer
      expect(input).not.toBeInstanceOf(ArrayBuffer)
      const open = vi.spyOn(Book.prototype, 'open').mockResolvedValue(undefined)

      new Book(input)

      expect(open).toHaveBeenCalledWith(input, undefined)
      open.mockRestore()
    })
    ...
    describe('synchronous setup throw', () => {
      ...
      it('should not throw from the constructor', async () => {
        let book!: Book
        expect(() => {
          book = new Book(new ArrayBuffer(8))
        }).not.toThrow()
        silenceSiblings(book)
      ...
      it('should reject rather than throw when open() is called directly', async () => {
        ...
        expect(() => {
          opening = book.open(new ArrayBuffer(8), 'binary')
        }).not.toThrow()
  ...
  describe('destroy lifecycle', () => {
    it('stops the rendition before releasing spine and archive resources', () => {
      const book = new Book()
      const order: string[] = []
      book.rendition = {
        destroy: () => order.push('rendition'),
      } as Book['rendition']
      vi.spyOn(book.spine, 'destroy').mockImplementation(() => {
        order.push('spine')
      })
      book.archive = {
        destroy: () => order.push('archive'),
      } as Book['archive']

      book.destroy()

      expect(order).toEqual(['rendition', 'spine', 'archive'])
    })
  })
})
