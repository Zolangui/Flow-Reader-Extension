...
describe("Core", () => {
	...
	describe("tree walking", () => {
		it("uses the root owner document instead of the reader shell document", () => {
			const foreign = document.implementation.createHTMLDocument("foreign");
			foreign.body.innerHTML = "<p>one <b>two</b></p>";
			const shellWalker = vi
				.spyOn(document, "createTreeWalker")
				.mockImplementation(() => {
					throw new TypeError("foreign root rejected");
				});
			const values: string[] = [];

			try {
				sprint(foreign.body, (node) => values.push(node.textContent ?? ""));
			} finally {
				shellWalker.mockRestore();
			}

			expect(values).toEqual(["one ", "two"]);
			expect(shellWalker).not.toHaveBeenCalled();
		});
	});
