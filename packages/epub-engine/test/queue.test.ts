...
describe('Queue', () => {
  ...
  describe('dequeue()', () => {
    ...
    it('should call task with queue context', async () => {
      ...
      q.enqueue(function (this: unknown) {
        capturedThis = this
      })
      await q.dequeue()
    ...
    it('should reject a synchronously throwing task and keep the queue usable', async () => {
      const q = createTestQueue()
      q.pause()
      const failed = q.enqueue(() => {
        throw new Error('sync failure')
      })
      const next = vi.fn(() => 'recovered')
      const recovered = q.enqueue(next)
    ...
    it('should continue after an enqueued promise rejects', async () => {
      const q = createTestQueue()
      q.pause()
      const gate = Promise.reject(new Error('gate failure'))
      const observedGate = q.enqueue(gate)
      const next = vi.fn(() => 'opened anyway')
      const recovered = q.enqueue(next)

      const running = q.run()
      await expect(observedGate).rejects.toThrow('gate failure')
      await expect(recovered).resolves.toBe('opened anyway')
      await running
      expect(next).toHaveBeenCalledOnce()
    })
...
describe('Task', () => {
  it('should wrap a callback-style function into a promise', async () => {
    const fn = function (
      _arg1: string,
      callback: (value: unknown, err: unknown) => void,
    ) {
      callback('result', null)
    }
    const wrapped = new Task(fn, []) as unknown as (
      ...args: unknown[]
    ) => Promise<unknown>
