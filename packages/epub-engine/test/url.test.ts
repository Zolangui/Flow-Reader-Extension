...
describe('Url', () => {
  ...
  describe('resolve()', () => {
    it('should resolve relative path to absolute URL', () => {
      ...
      expect(url.resolve('chapter.xhtml')).toBe(
        'http://example.com/OPS/text/chapter.xhtml',
      )
    })

    it('should resolve .. traversal', () => {
      const url = new Url('http://example.com/OPS/text/')
      expect(url.resolve('../images/cover.jpg')).toBe(
        'http://example.com/OPS/images/cover.jpg',
      )
    })
