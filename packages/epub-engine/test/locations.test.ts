...
describe('Locations', () => {
  describe('#parse', () => {
    const chapter = fs.readFileSync(
      path.join(fixturesDir, 'locations.xhtml'),
      'utf8',
    )
    ...
    it('yields at least one location for an image-only section', () => {
      ...
      const doc = core.parse(imageOnly, 'application/xhtml+xml')
      const contents = doc.documentElement
      const locations = new Locations()
      const result = locations.parse(contents, '/6/4[chap01ref]', 100)
    ...
    it("anchors a section's trailing location inside its final node", () => {
      ...
      const twoNodes =
        '<html xmlns="http://www.w3.org/1999/xhtml"><body><p>aaaaaaaaaaaaaaaaaaaaaa</p><p>bb</p></body></html>'
      const doc = core.parse(twoNodes, 'application/xhtml+xml')
      const contents = doc.documentElement
      const locations = new Locations()
      const result = locations.parse(contents, '/6/4[chap01ref]', 20)
  ...
  describe('load→save round-trip', () => {
    it('round-trips locations through load and save', () => {
      ...
      const arr = [
        'epubcfi(/6/4!/4/2/1:0)',
        'epubcfi(/6/4!/4/2/1:150)',
        'epubcfi(/6/4!/4/2/3:50)',
      ]
      loc.load(arr)
  ...
  describe('locationFromHref()', () => {
    ...
    function createSpined(
      sections: Record<string, Partial<Section>> = reflowable,
    ): Locations {
    ...
    describe('against a real book', () => {
      ...
      it('resolves to a location inside the target section', () => {
        ...
        expect(new EpubCFI(generated[index - 1]!).spinePos).toBeLessThan(
          section.index!,
        )
      })
      ...
      it("ignores the fragment, resolving to the section's first location", () => {
        expect(
          book.locations.locationFromHref('chapter_005.xhtml#pgepubid00008'),
      ...
      it('resolves from loaded locations without reloading sections', async () => {
        ...
        expect(other.locations.locationFromHref('chapter_005.xhtml')).toBe(
          book.locations.locationFromHref('chapter_005.xhtml'),
        )
      })
  ...
  describe('isPrePaginated()', () => {
    ...
    it('should follow the book layout when the section has no override', () => {
      expect(withLayout('pre-paginated').isPrePaginated(sectionWith([]))).toBe(
        true,
      )
      expect(withLayout('reflowable').isPrePaginated(sectionWith([]))).toBe(
        false,
      )
    })
