...
describe('TextMeasurer', () => {
  ...
  describe('cumDocWidth', () => {
    function prepareMockNodes(
      measurer: TextMeasurer,
      texts: string[],
    ): import('../src/utils/text-measurer').PreparedNode[] {
    ...
    it('should compute monotonically increasing cumDocWidth', () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        if (i > 0) {
          expect(nodes[i]!.cumDocWidth).toBeGreaterThan(
            nodes[i - 1]!.cumDocWidth,
          )
        }
      }
  ...
  describe('findNodeIndex()', () => {
    ...
    it('should return last index for target beyond total', () => {
      const prepared = [{ cumDocWidth: 50 }, { cumDocWidth: 120 }] as any[]
