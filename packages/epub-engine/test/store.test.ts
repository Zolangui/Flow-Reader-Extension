...
describe('Store', () => {
  ...
  describe('handleResponse()', () => {
    ...
    it('should parse XML', () => {
      const store = new Store('test-handle')
      const result = store.handleResponse(
        '<root><item/></root>',
        'xml',
      ) as Document
  ...
  describe('retrieve()', () => {
    it('should reject for missing file', async () => {
      ...
      await expect(
        store.retrieve('http://example.com/nonexistent.txt'),
      ).rejects.toBeDefined()
    })
  })
  ...
  describe('createUrl()', () => {
    ...
    it('should support base64 option', async () => {
      ...
      const url = await store.createUrl('http://example.com/b64url.txt', {
        base64: true,
      })
