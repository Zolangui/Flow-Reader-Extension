...
import { isPresentationRuntimeNode } from '../src/presentation-marker'
...
function createContents(bodyHtml?: string): {
  contents: Contents
...
describe('Contents', () => {
  ...
  describe('addStylesheet()', () => {
    it('should append a link element to document head', async () => {
      ...
      expect(isPresentationRuntimeNode(link!)).toBe(true)
    ...
    it('should not add duplicate link element', () => {
      ...
      // cleanup
      document
        .querySelectorAll("link[href='http://example.com/dup.css']")
        .forEach((el) => el.remove())
    })

    it('deduplicates a stylesheet while the first request is pending', async () => {
      const { contents } = createContents()
      const first = contents.addStylesheet('http://example.com/pending.css')
      const second = contents.addStylesheet('http://example.com/pending.css')
      expect(second).toBe(first)
      const links = document.querySelectorAll(
        "link[href='http://example.com/pending.css']",
      )
    ...
    it('compares special-character URLs without building an unsafe selector', async () => {
      const { contents } = createContents()
      const src = "http://example.com/theme.css?name=a'b]c"
      const promise = contents.addStylesheet(src)
      const link = Array.from(document.querySelectorAll('link')).find(
        (candidate) => candidate.getAttribute('href') === src,
      )
    ...
    it('settles and removes a pending stylesheet when contents is destroyed', async () => {
      const { contents } = createContents()
      const promise = contents.addStylesheet('http://example.com/hanging.css')
      contents.destroy()
      await expect(promise).resolves.toBe(false)
      expect(
        document.querySelector("link[href='http://example.com/hanging.css']"),
      ).toBeNull()
    })
  })
  ...
  describe('addStylesheetCss()', () => {
    it('should create a style element with textContent', () => {
      const { contents } = createContents()
      const result = contents.addStylesheetCss(
        'body { color: red; }',
        'test-key',
      )
      ...
      expect(isPresentationRuntimeNode(styleEl!)).toBe(true)
      styleEl!.remove()
    })
  ...
  describe('addStylesheetRules()', () => {
    ...
    it('should handle array format rules', () => {
      const { contents } = createContents()
      contents.addStylesheetRules([['p', ['color', 'red']]], 'arr-key')
      const styleEl = document.getElementById(
        'epubjs-inserted-css-arr-key',
      ) as HTMLStyleElement
      expect(styleEl).not.toBeNull()
      expect(styleEl.sheet!.cssRules.length).toBeGreaterThan(0)
      styleEl.remove()
  ...
  describe('resizeCheck()', () => {
    ...
    it('should not emit resize when size is unchanged', () => {
      ...
      contents._size = {
        width: contents.textWidth(),
        height: contents.textHeight(),
      }
      const handler = vi.fn()
      contents.on('resize', handler)
      contents.resizeCheck()
  ...
  describe('triggerSelectedEvent()', () => {
    it('ignores a live selection cleared before getRangeAt', () => {
      const { contents } = createContents('<p>Selection race</p>')
      const selection = {
        rangeCount: 1,
        getRangeAt: () => {
          throw new DOMException('Index 0 is out of range', 'IndexSizeError')
        },
      } as unknown as Selection

      expect(() => contents.triggerSelectedEvent(selection)).not.toThrow()
    })
  })
