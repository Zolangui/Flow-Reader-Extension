...
describe('replacements', () => {
  describe('replaceBase()', () => {
    it('should insert a <base> element when none exists', () => {
      ...
      expect(base!.getAttribute('href')).toBe(
        'http://example.com/OPS/chapter.xhtml',
      )
    })
    ...
    it('should no-op when doc is falsy', () => {
      expect(() =>
        replaceBase(null as unknown as Document, { url: 'http://x.com' }),
      ).not.toThrow()
    })
  })
  ...
  describe('replaceCanonical()', () => {
    ...
    it('should no-op when doc is falsy', () => {
      expect(() =>
        replaceCanonical(null as unknown as Document, { canonical: 'x' }),
      ).not.toThrow()
    })
  })
  ...
  describe('replaceMeta()', () => {
    ...
    it('should no-op when doc is falsy', () => {
      expect(() =>
        replaceMeta(null as unknown as Document, { idref: 'x' }),
      ).not.toThrow()
    })
  })
