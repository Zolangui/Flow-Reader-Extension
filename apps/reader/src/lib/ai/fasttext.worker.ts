...
type LidModel = {
    load: () => Promise<any>
    identify: (text: string) => Promise<{ alpha2?: string | null; alpha3?: string | null }>
}

function debugLog(message: string, data?: Record<string, any>) {
    if (!DEBUG) return
    try {
        self.postMessage({ type: 'fasttext-debug', data: { message, ...(data || {}) } })
    } catch {
        // ignore
    }
}
...
function resolveAssetURL(path: string) {
    ...
    return typeof getURL === 'function' ? getURL(path) : `${origin}/${path.replace(/^\//, '')}`
}
...
async function getModel(): Promise<LidModel> {
    ...
    modelPromise = (async () => {
        // Import the smallest possible entrypoint to reduce the module graph and avoid Firefox MV3
        // edge-cases around ESM re-export chains.
        debugLog('init_start', { module: '/fastText/models/language-identification/common.mjs' })

        const moduleUrl = resolveAssetURL('fastText/models/language-identification/common.mjs')
...
self.onmessage = (e: MessageEvent) => {
    ...
    void (async () => {
        try {
            const input = typeof text === 'string' ? text.trim() : String(text || '').trim()
