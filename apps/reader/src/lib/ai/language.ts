...
let fastTextStatus: 'unknown' | 'downloading' | 'ready' | 'warning' | 'error' =
  loadFastTextReadySync() ? 'ready' : 'unknown'
let fastTextLastError: string | null = null
let fastTextWarning: string | null = null
let fastTextReasonCode: ModelReasonCode = 'none'
const FASTTEXT_BACKEND = 'fasttext-wasm'
const FASTTEXT_MODEL_ID = 'lid.176.ftz'
...
function emitFastTextStatus(
  status: typeof fastTextStatus,
  reasonCode: ModelReasonCode = 'none',
  message?: string,
) {
  fastTextStatus = status
  fastTextReasonCode = reasonCode
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('fasttext-status', {
        detail: {
          ...createModelHealthState(status, {
            reasonCode,
            message,
            backend: FASTTEXT_BACKEND,
            modelId: FASTTEXT_MODEL_ID,
          }),
          warning: fastTextWarning,
        },
      }),
    )
  }
  if (status === 'ready') {
    try {
      if (typeof window !== 'undefined')
        window.localStorage.setItem(FASTTEXT_CACHE_KEY, '1')
    } catch {
      // ignore
    }
...
function emitFastTextError(message: string, stage?: string) {
  ...
  emitFastTextStatus('error', normalized.reasonCode, normalized.message)
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('fasttext-error', {
        detail: {
...
function ensureFastTextWorker() {
  if (fastTextWorker || fastTextUnavailable || typeof Worker === 'undefined')
    return

  try {
    fastTextWorker = new Worker(
      new URL('./fasttext.worker.ts', import.meta.url),
      { type: 'module' },
    )
  } catch {
  ...
  fastTextWorker.onmessage = (e: MessageEvent) => {
    ...
    if (type === 'fasttext-ready') {
      emitFastTextStatus('ready', 'download_complete')
      return
    }
    if (type === 'fasttext-debug') {
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('fasttext-debug', { detail: e.data.data }),
        )
      }
    ...
    if (type === 'fasttext-error') {
      emitFastTextError(
        String(e?.data?.data?.message || 'fasttext_error'),
        e?.data?.data?.stage,
      )
      return
    }
...
export function getFastTextWarning() {
  return fastTextWarning
}

export function getFastTextReasonCode() {
  return fastTextReasonCode
}
...
async function hydrateFastTextReady() {
  ...
  try {
    ...
    if (ready && fastTextStatus === 'unknown') {
      emitFastTextStatus('ready', 'cache_hit')
    }
  } catch {
...
export async function preloadFastText(): Promise<void> {
  ...
  const shouldShowDownloading =
    fastTextStatus === 'unknown' || fastTextStatus === 'error'
  if (shouldShowDownloading)
    emitFastTextStatus('downloading', 'download_started')
  if (fastTextWarning) fastTextWarning = null
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      ...
      emitFastTextStatus('warning', 'preload_timeout', 'timeout')
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('fasttext-debug', {
            detail: {
              message: 'preload_timeout',
              timeoutMs: FASTTEXT_PRELOAD_TIMEOUT_MS,
            },
          }),
        )
      }
      console.warn('[FastText]', {
        stage: 'preload',
        message: fastTextLastError,
      })
      resolve()
    ...
    fastTextPending.set(id, () => {
      clearTimeout(timeout)
      if (fastTextWarning === 'preload_timeout') {
        fastTextWarning = null
      }
      resolve()
    })
 ...
 */
export function normalizeLangForRAG(
