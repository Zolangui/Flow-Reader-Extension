...
let slmStatus: 'unknown' | 'downloading' | 'ready' | 'warning' | 'error' = loadCachedSync()
...
function emitStatus(status: typeof slmStatus) {
    slmStatus = status
    if (status === 'ready' || status === 'warning') {
        saveCached()
    }
    if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('slm-status', { detail: { status, warning: slmWarning } }))
    }
}
...
function ensureWorker() {
    if (slmWorker || slmUnavailable || typeof Worker === 'undefined') return
    try {
        slmWorker = new Worker(new URL('./slm.worker.ts', import.meta.url), { type: 'module' })
    } catch (err) {
        slmUnavailable = true
        ...
        if (typeof window !== 'undefined') {
            window.dispatchEvent(
                new CustomEvent('slm-error', {
                    detail: { stage: 'init', message: slmLastError },
                }),
            )
        }
    ...
    slmWorker.onmessage = (e: MessageEvent) => {
        ...
        if (type === 'slm-ready') {
            ...
            emitStatus(slmWarning ? 'warning' : 'ready')
            return
        }
        if (type === 'slm-debug') {
            if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('slm-debug', { detail: e.data.data }))
            }
            ...
            if (dbg?.message === 'download_complete' || dbg?.message === 'cache_hit') {
                saveCached()
        ...
        if (type === 'slm-error') {
            ...
            const stack = typeof e?.data?.data?.stack === 'string' ? e.data.data.stack : undefined
            ...
            if (stage === 'preload' && message === 'timeout') {
                setSlmWarning('preload_timeout')
                emitStatus('warning')
                if (typeof window !== 'undefined') {
                    window.dispatchEvent(
                        new CustomEvent('slm-error', {
                            detail: { stage, message, stack },
                        }),
                    )
                }
        ...
        if (type === 'slm-progress') {
            ...
            if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('slm-progress', { detail: e.data.data }))
            }
            return
        }
...
function shouldRewrite(query: string, history: RewriteHistoryMessage[]): boolean {
    ...
    const markers = [
        'entao', 'so', 'and', 'e', 'mas', 'but', 'por que', 'porque', 'why', 'como', 'how',
    ]
    if (markers.some(m => normalized === m || normalized.startsWith(`${m} `))) return true

    const pronouns = ['isso', 'isto', 'ele', 'ela', 'eles', 'elas', 'it', 'this', 'that', 'they', 'them']
    if (pronouns.some(p => words.includes(normalizeDiacritics(p)))) return true

    return normalized.endsWith('?')
}

async function rewriteWithWorker(query: string, history: RewriteHistoryMessage[]): Promise<string | null> {
    if (slmUnavailable) return null
    ensureWorker()
...
async function classifyWithWorker(query: string, history: RewriteHistoryMessage[]): Promise<IntentClassification | null> {
    if (slmUnavailable) return null
    ensureWorker()
    if (!slmWorker) return null
    if (slmStatus === 'unknown') emitStatus('downloading')
    ...
    return new Promise((resolve) => {
        ...
        pending.set(id, (data) => {
            ...
            const primaryQuery = typeof payload.primaryQuery === 'string'
            ...
            if (typeof window !== 'undefined') {
                window.dispatchEvent(
                    new CustomEvent('slm-debug', {
                        detail: { message: 'preload_timeout', timeoutMs: PRELOAD_TIMEOUT_MS },
                    }),
                )
            }
