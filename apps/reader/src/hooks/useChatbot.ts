...
function rrfFuse(
  ...
  return Array.from(itemByIdx.entries())
    .map(([idx, it]) => ({
      ...it,
      score: scoreByIdx.get(idx) || it.score || 0,
    }))
...
export function useChatbot() {
  ...
  useEffect(() => {
    ...
    let sessions = Array.isArray(book.chatSessions)
      ? [...book.chatSessions]
      : []
    let activeChatId = book.activeChatId

    if (sessions.length === 0) {
      const legacyMessages = Array.isArray(book.chatHistory)
        ? book.chatHistory
        : []
  ...
  const sendMessage = async (
    ...
    try {
      ...
      const { depth, scope } = isSelectionAction
        ...
        : {
            depth: (options.deeper ? 'deep' : settings.answerDepth) as
              | 'short'
              | 'balanced'
              | 'deep',
            scope: settings.aiScope,
          }
      ...
      for (let qi = 0; qi < queries.length; qi++) {
        ...
        const items = await rag.retrieveContext(
          book.id,
          q,
          retrievalParams.topK,
          {
      ...
      const selectionPrompts = options.action
        ? buildSelectionActionPrompt(
            options.action,
            content,
            dedupedContext.map((c) => c.content).join('\n'),
            lang,
          )
        : null
      ...
      if (multiIntentMode) {
        const intentLine = classifiedIntents
          .map((i) => `${i.type}: ${i.query}`)
          .join(' | ')
      ...
      const stream = llm.streamResponse(
        systemPrompt,
        userPrompt,
        controller.signal,
        history,
      ...
      for await (const chunk of stream) {
        ...
        window.dispatchEvent(
          new CustomEvent('chatbot-stream', {
      ...
      if (
        ...
        const audit = auditResponseCitations(final, dedupedContext, {
          requireQuotes: true,
          minQuoteChars: 16,
          minMatchedQuotes: 1,
        })
        ...
        if (!audit.ok) {
          try {
            const historyForRepair = stateRef.current.messages.map((m) => ({
              role: m.role,
              content: m.content,
            }))
...
Citation Reliability Mode (MANDATORY):
- Include 1-2 short verbatim quotes from the provided book excerpts.
- Every quote must be grounded in the same chunks listed in the Sources footer.
- Keep NO inline citation IDs. Keep IDs only in final Sources line.
- If a claim cannot be supported by provided excerpts, remove it.`

            const repairUserPrompt = `${userPrompt}

Rewrite your previous answer to fix citation grounding issues.
Previous answer:
---
${final}
---

Audit failures: ${audit.reasons.join(', ') || 'unknown'}.
Return only the corrected final answer.`

            const repaired = await llm.generateResponse(
              repairSystemPrompt,
              repairUserPrompt,
              historyForRepair,
            )
            const repairedFinal = normalizeCitationsToFooter(repaired)
            const repairedAudit = auditResponseCitations(
              repairedFinal,
              dedupedContext,
              {
                requireQuotes: true,
            ...
            const repairedScore =
              repairedAudit.sourceIds.length * 2 +
              repairedAudit.matchedQuotes -
              repairedAudit.reasons.length * 2
            const baseScore =
              audit.sourceIds.length * 2 +
              audit.matchedQuotes -
              audit.reasons.length * 2
            if (repairedAudit.ok || repairedScore >= baseScore) {
              final = repairedFinal
            }
          } catch (repairErr: any) {
            console.warn(
              '[Citation Audit] Repair attempt failed:',
        ...
        if (bookTab) {
          ...
          const updatedSession: ChatSessionRecord = {
            ...
            title: deriveChatTitle(
              nextMessages,
              idx >= 0 ? sessions[idx].title : undefined,
            ),
          }
