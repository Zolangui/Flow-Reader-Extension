export default {
  ...
  'ai.confirm_reset': '重置 AI 参数为默认值？（您的 API 密钥和基准 URL 将被保留）',
  ...
  'ai.settings.model_hint.gemini': '性能首选：gemini-1.5-pro。速度首选：gemini-1.5-flash。',
  'ai.settings.model_hint.openai': '性能首选：gpt-4o。速度首选：gpt-4o-mini。',
  'ai.settings.model_hint.anthropic': '强烈推荐：claude-3-5-sonnet-latest。',
  'ai.settings.model_hint.local': '连接到 Ollama 或 LM Studio 等本地推理服务器。数据不会离开您的机器。',
  'ai.settings.model_hint.custom': '连接到任何 OpenAI 兼容的 API 代理或专用端点（例如 OpenRouter、Anyscale）。',
  ...
  'ai.settings.privacy_desc': '您的 API 密钥和聊天历史记录以加密方式存储在本地。我们永远无法看到您的数据。',
  'ai.settings.start_assistant': '启动助手',
  'ai.settings.save_changes': '保存更改',
  'ai.settings.enter_api_key_first': '请先输入 API 密钥',
  'ai.settings.fetch_models_error': '无法获取模型。请检查您的 API 密钥或网络连接。',
  'ai.settings.adaptive_context_hint': '定义 AI 的性格和界限。启用“自适应上下文”后，系统将根据书籍元数据自动优化。',
  ...
  'ai.chatbot.empty_state_desc': '可以问关于当前书籍的任何问题！助手会使用 RAG 查找相关章节和引用。',
  'ai.chatbot.index_description': '这本书尚未被 AI 处理。现在建立索引以启用语义搜索和精准引用。',
